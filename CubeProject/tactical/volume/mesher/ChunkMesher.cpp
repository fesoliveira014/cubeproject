...
namespace tactical
{
	namespace volume
	{
		namespace mesher
		{
			void Mesher::GenerateSurfaceNets(Chunk& chunk)
			{
				int dims = chunk.GetSize();
				std::vector<glm::vec3> vertices;
				std::vector<int> buffer((dims+1) * (dims+1) * 2);

				glm::vec3 posOffset = chunk.GetPosition();
				...
				for (int k = 0; k < dims; ++k) {
					for (int j = 0; j < dims; ++j) {
						for (int i = 0; i < dims; ++i) {
							...
							byte vs[8] = {
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(-1, -1, -1)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(0, -1, -1)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(-1, 0, -1)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(0, 0, -1)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(-1, -1, 0)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(0, -1, 0)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(-1, 0, 0)),
								chunk.GetChunkManager()->GetVoxel(posOffset + p + glm::vec3(0, 0, 0))
							};
							...
							/*do_edge(vs[0], vs[1], 0, p + glm::vec3(-1, -1, -1));
							do_edge(vs[2], vs[3], 0, p + glm::vec3(-1, 0, -1));
							do_edge(vs[4], vs[5], 0, p + glm::vec3(-1, -1, 0));
							do_edge(vs[6], vs[7], 0, p + glm::vec3(-1, 0, -1));
							do_edge(vs[0], vs[2], 1, p + glm::vec3(-1, -1, 0));
							do_edge(vs[1], vs[3], 1, p + glm::vec3(0, -1, -1));
							do_edge(vs[4], vs[6], 1, p + glm::vec3(-1, -1, 0));
							do_edge(vs[5], vs[7], 1, p + glm::vec3(0, -1, 0));
							do_edge(vs[0], vs[4], 2, p + glm::vec3(-1, -1, -1));
							do_edge(vs[1], vs[5], 2, p + glm::vec3(0, -1, -1));
							do_edge(vs[2], vs[6], 2, p + glm::vec3(-1, 0, -1));
							do_edge(vs[3], vs[7], 2, p + glm::vec3(0, 0, -1));
*/
			...
			void Mesher::Greedy(Chunk& chunk)
			{
				// cleanup
			...
			void Mesher::NaiveWithCulling(Chunk& chunk)
			{
				//delete chunk.GetMesh()->vao;
				//delete chunk.GetMesh()->ibo;
				//chunk.GetMesh()->vertices.clear();
